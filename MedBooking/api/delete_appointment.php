...
try {
    ...
    $stmt = $pdo->prepare("SELECT a.doctor_id, a.patient_id, a.appointment_date, a.appointment_time, a.google_calendar_event_id, p.full_name as patient_name, p.email as patient_email, d.full_name as doctor_name, d.email as doctor_email FROM appointments a JOIN users_patients p ON a.patient_id = p.patient_id JOIN users_doctors d ON a.doctor_id = d.doctor_id WHERE a.appointment_id = ? AND a.doctor_id = ?");
    $stmt->execute([$appointmentId, $doctorId]);
    $appointment = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$appointment) {
        throw new Exception("Appointment not found or does not belong to this doctor.", 404);
    }
        ...
        if ($doctor && !empty($doctor['google_refresh_token'])) {
            try {
                ...
                if (strpos($google_e->getMessage(), 'invalid_grant') !== false) {
                    ...
                    error_log("Failed to delete Google Calendar event (ID: {$appointment['google_calendar_event_id']}): " . $google_e->getMessage());
                }
                // --- FIM DA MELHORIA ---
            }
        }
    ...
    $stmt_delete = $pdo->prepare("DELETE FROM appointments WHERE appointment_id = ?");
    $stmt_delete->execute([$appointmentId]);
    ...
    try {
        ...
        $emailHtmlBody = getAppointmentCancellationEmailHtml(
            ...
            "Doctor" // Deletado pelo médico
        );
        
        $altBodyText = "Appointment Canceled: The appointment between " . $appointment['patient_name'] . " and Dr. " . $appointment['doctor_name'] . " on " . $appointmentDateFormatted . " at " . $appointmentTimeFormatted . " has been canceled by the Doctor.";
    ...
    $pdo->commit();
    sendJsonResponse(["success" => true, "message" => "Appointment deleted successfully."]);
    ...
    error_log("delete_appointment.php Error: " . $e->getMessage());
    sendJsonResponse(["success" => false, "message" => "Error deleting appointment: " . $e->getMessage()], $statusCode);
}
?>