...
try {
    ...
    $redirectUri = trim($_ENV['GOOGLE_REDIRECT_URI_PATIENT'], '"');
    ...
    $stmt = $pdo->prepare("SELECT * FROM users_patients WHERE email = ?");
    $stmt->execute([$email]);
    $user = $stmt->fetch();
    ...
    if ($user) {
        ...
        if (!$hasCustomPicture && $user['profile_picture_url'] !== $pictureUrl) {
            $updateStmt = $pdo->prepare("UPDATE users_patients SET profile_picture_url = ? WHERE patient_id = ?");
            $updateStmt->execute([$pictureUrl, $user['patient_id']]);
        }
    } else {
        $insertStmt = $pdo->prepare(
            "INSERT INTO users_patients (full_name, email, profile_picture_url, is_active) VALUES (?, ?, ?, 1)"
        );
        $insertStmt->execute([$fullName, $email, $pictureUrl]);
    }
    
    $finalUserStmt = $pdo->prepare("SELECT patient_id, full_name, email, is_active FROM users_patients WHERE email = ?");
    $finalUserStmt->execute([$email]);
    $dbUser = $finalUserStmt->fetch(PDO::FETCH_ASSOC);

    if (!$dbUser) {
        throw new Exception("Critical failure to get final patient record.");
    }

    $jwtToken = generateJwtToken($dbUser['patient_id'], 'patient', $dbUser['email'], $_ENV['JWT_SECRET_KEY']);

    $userPayload = [
        "id" => (int)$dbUser['patient_id'],
